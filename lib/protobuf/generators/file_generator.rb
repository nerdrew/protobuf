  ...
  module Generators
    class FileGenerator < Base
      ...
      def map_extensions(descriptor, namespaces)
        ...
        # Record all the message descriptor names we encounter (should be the whole tree).
        if descriptor.is_a?(::Google::Protobuf::EnumDescriptorProto)
          if fully_qualified_token?(descriptor.name)
            @known_enums[descriptor.name] = descriptor
          else
            fully_qualified_namespace = ".#{namespaces.join('.')}"
            @known_enums[fully_qualified_namespace] = descriptor
          end
        elsif descriptor.is_a?(::Google::Protobuf::DescriptorProto)
          if fully_qualified_token?(descriptor.name)
            @known_messages[descriptor.name] = descriptor
          else
            fully_qualified_namespace = ".#{namespaces.join('.')}"
            @known_messages[fully_qualified_namespace] = descriptor
          end
        end

        return if descriptor.is_a?(::Google::Protobuf::EnumDescriptorProto)
      ...
      def print_package(&block)
        ...
        if namespaces.empty? && ENV.key?('PB_DEFAULT_PACKAGE_NAME')
      ...
      def eval_message_code(fully_qualified_namespace, fields = [])
        group = GroupGenerator.new(0)
        group.add_extended_messages(fully_qualified_namespace => fields)
        print group.to_s
        eval_code
      end
