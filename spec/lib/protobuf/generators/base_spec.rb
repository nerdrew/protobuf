  ...
  describe '#serialize_value' do
    ...
    it 'serializes messages' do
      expect(generator.serialize_value(MyMessage3.new(
        ...
        # Default values do not get serialized in messages, so ':goat => ::MyEnum::FOO'
        # is not included, since ::MyEnum::FOO is the default MyEnum
        '{ :foo => "space", :bar => [{ '\
        ':foo => "station", :bar => { :foo => "orbit" }, :boom => 123, :fire => 3.5 '\
